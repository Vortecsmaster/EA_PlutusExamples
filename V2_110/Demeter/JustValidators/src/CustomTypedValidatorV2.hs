...
give amount = do                                                                            -- mustPayToTheScriptWithDatumInTx
    let tx = mustPayToTheScriptWithDatumHash () $ Ada.lovelaceValueOf amount               -- Correction pending on ...Typed version for one script, This Tx needs an output, thats its going to be the Script Address, Datum MUST be specified, so unit ().
