...
const Helios: NextPage = () => {
  ...
  const lockUtxo = async () => {
    if (lucid) {
      ...
      const tx = await lucid
        .newTx()
        .payToContract(simpleValidatorAddress, Datum(19), { lovelace:BigInt (50000000) })
        .payToContract(simpleValidatorAddress, Datum(20), { lovelace:BigInt (70000000) })
        .payToContract(simpleValidatorAddress, Datum(23), { lovelace:BigInt (500000000) })
        .payToContract(simpleValidatorAddress, Datum(23), { lovelace:BigInt (200000000) })
        .complete();
  ...
  const unlockUtxo = async () => {
    if (lucid) {
      ...
      const utxo = (await lucid.utxosAt(simpleValidatorAddress)).filter((utxo) =>
        utxo.datumHash === Datum(23)
      );
