...
{-# INLINABLE justValidator #-}
justValidator :: BuiltinData -> BuiltinData -> BuiltinData -> ()
justValidator datum redeemer _ 
    | redeemer == datum             = ()
    | redeemer Builtins.mkI 999     = ()
    | otherwise                   = traceError "Wrong Redeemer"


{-# INLINABLE justLoan #-}
justLoan :: BuiltinData -> BuiltinData -> BuiltinData -> ()
justLoan datum action _ 
    | action == REQUEST     = {find the borrower NFT && verify enough collateral && validate enough to cover collatera and fees}
    | action == LEND        = {validate lender NFT && lender provided enough value to cover loan && verify enough collateral available && validate enough to cover collatera and fees}
    | action == REPAYMENT   = ...
    | action == MARGINCALL  = ...
    | otherwise                   = traceError "Unknown action"
...
test = runEmulatorTraceIO $ do
    ...
    Extras.logInfo $ "End of Simulation at slot " ++ show s                                                                   -- MakeKnown currencies for the playground to have some ADA available
